                                                            ...
                                                            event.new(0,{"WHAT":1,"MY GENERAL, WHAT ARE YOU SAYING?":1.3,"AFTER THEY HUMILIATED YOU ON THE VERY DAY OF YOUR ARRIVAL?!":1.3},load("res://sfx/sVoice.wav"),0.06,load("res://fonts/MARYJSC_.ttf"),[],"","",[],
                                                                event.new(0,{"And why do you think that is?":1.5,"I mean, why COULD they do that?":1.3,"I'll tell ya why.":1.3,"Those... creatures.":1,"They are like rats in your ship's kitchen.":1.3,"One of them is just a hungry, barbaric little creature.":1.3,"But once they get breedin'... oh boy.":0.7,"There'll be so much of them, extermination becomes nearly impossible.":1.7,"If we tried to attack them now, we'd be gone like ship's cheese.":2,"That's why our only hope of victory is our superior intellect.":1.2,"If we want to win against vermin, we need to moke some mousetraps.":2,"Capiche?":1},load("res://sfx/gVoice.wav"),0.08,load("res://fonts/MJALTER_.ttf"),[],"","",[],
                                                                    event.new(0,{"Y-Yessir!":0.5,"We'll start gathering potential inventors to the navy's weaponry division right away!":2},load("res://sfx/sVoice.wav"),0.06,load("res://fonts/MARYJSC_.ttf"),[],"","",[],
                                                                        event.new(0,{"Good.":0.5,"Tell 'em to work as fast as possible.":1,"Right now our only protection from them is the sea.":1.5,"And who knows how fast the rats will learn how to swim.":2},load("res://sfx/gVoice.wav"),0.07,load("res://fonts/MJALTER_.ttf"),[],"","hubScene",[
                                                                            event.new(0,{},null,0,null,[load("res://backgrounds/yDefault1.png"),load("res://backgrounds/yDefault2.png")],"","",[],
                                                                                event.new(0, {"*sigh* This is bad news.":1,"Somehow, when one side starts comparing the other to vermin,":1.5,"It always means bad fate is to be expected...":2.2,"Still, mingling with lower planes should only be treated as a last resort.":1.5,"There is no telling how they will react to finding out there are beings above them,":1,"So it may just make the situation worse!":2},load("res://sfx/yVoice.wav"),0.07,load("res://fonts/MARYJO__.ttf"),[],"","main",[
                                                                                     event.new(0,{},null,0,null,[],"","islandScene",[
                                                                                         event.new(50,{},null,0,null,[load("res://backgrounds/islandGeneralDead1.png"),load("res://backgrounds/islandGeneralDead2.png")],"","egyptScene",[
                                                                                             event.new(0,{},null,0,null,[load("res://backgrounds/desertDescendantSpeech1.png"),load("res://backgrounds/desertDescendantSpeech2.png")],"")
                                                                                         ]),
                                                                                         event.new(69,{},null,0,null,[load("res://backgrounds/islandDescendantPonder1.png"),load("res://backgrounds/islandDescendantPonder2.png")],"","",[]),
